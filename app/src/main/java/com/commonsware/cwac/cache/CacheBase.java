...
abstract public class CacheBase<K, V> {
	...
	public static final int CACHE_PATH_DISK = 4;// co duong dan ko ma hoa md5 o local sdcard 
	...
	public CacheBase(File cacheRoot, DiskCachePolicy policy, int maxSize) {
		...
		if (GlobalInfo.getInstance().getActivityContext()!=null)
			activityId = GlobalInfo.getInstance().getActivityContext().toString();
		VTLog.e("PhucNT4", "activity Id "+ activityId);
	...
	public void recyleOneBitmap(Object key) {		
	...
	public class CacheCleanTask extends
		...
		protected Void doInBackground(DiskCachePolicy... policies) {
			try {
				...
				VTLog.e(TAG, "Exception cleaning cache" + t);
			}
