...
/**
 * Row hiển thị thông tin KH không phát sinh doanh số trong tháng cho GSBH
 * GSBHCustomerNotPSDSInMonthRow.java
 * 
 * @author: QuangVT
 * @version: 1.0 
 */
public class GSNPPCustomerNotPSDSInMonthRow extends TableRow implements
		OnClickListener { 
	...
	public static HeaderTableInfo[] getTableHeader(Context con) {
		if (TABLE_HEADER == null) {
			GSNPPCustomerNotPSDSInMonthRow row = new GSNPPCustomerNotPSDSInMonthRow(con);
	 ...
	 * @param context
	 */
	private GSNPPCustomerNotPSDSInMonthRow(Context context) {
		super(context);
		LayoutInflater vi = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		view = vi.inflate(R.layout.layout_supervisor_customer_not_psds_in_month_report_row, this, false);
	 ...
	 * @param action
	 */
	public GSNPPCustomerNotPSDSInMonthRow(Context context,
	...
	public void render(int pos, CustomerNotPsdsInMonthReportViewDTO.CustomerNotPsdsInMonthReportItem info) {
		...
		final String strCus 	   = info.customerCode.substring(0, 3) + " - " + info.customerName;
