...
/**
 * TNPG General Statistic View TNPGGeneralStatisticView.java
 * 
 * @version: 1.0
 * @since: 08:32:06 20 Jan 2014
 */
public class TNPGGeneralStatisticView extends BaseFragment implements
		OnEventControlListener, VinamilkTableListener, OnTouchListener,
		OnItemSelectedListener {
	// TAG of fragment
	public static final String TAG = TNPGGeneralStatisticView.class.getName();
	// parent Activity
	...
	// BEGIN NEW INSTANCE
	public static TNPGGeneralStatisticView newInstance(Bundle b) {
		TNPGGeneralStatisticView instance = new TNPGGeneralStatisticView();
		instance.setArguments(b);
		return instance;
	}

	public static TNPGGeneralStatisticView getInstance(Bundle b) {
	...
	private void initView(View view) {
		...
		// moi loai request co row + header rieng
		switch (typeRequest) {
		case NVBHGeneralStatisticsView.TYPE_REQUEST_TNPG: {
			initHeaderTable(tbStatistics, new TNPGGeneralStatisticsRow(parent, this));
		}
			break;
	...
	private void renderLayout() {
		...
		if (dto != null) {
			...
			// show list staff in combo box
			if (dto.listStaffInfo != null) {

				listStaff = dto.listStaffInfo.arrList;
				String[] arrStaff = {};
				List<String> listStr = new ArrayList<String>();
				for (int i = 0; i < listStaff.size(); i++) {
					StaffItem info = listStaff.get(i);
					listStr.add(info.name);
				}
				arrStaff = listStr.toArray(arrStaff);

				isCodeSelectedSpniner = true;
				SpinnerAdapter adapter = new SpinnerAdapter(parent, R.layout.simple_spinner_item, arrStaff);
				spinnerNVTT.setAdapter(adapter);
				// da load xong staff list
			}
			if (dto.reportInfo != null) {

				// render header info
				renderHeaderCustomerInfo(dto.reportInfo.customerInfo);

				// render progress
				if (dto.reportInfo.progressSoldInfo != null) {
					// show progress info
					renderProgress(dto.reportInfo.progressSoldInfo);
					progressSold = dto.reportInfo.progressSoldInfo.progressSold;
				}

				if (dto.reportInfo.listInfo != null) {
					// render row
					for (int i = 0; i < dto.reportInfo.listInfo.size(); i++) {
						GeneralStatisticsInfo info = dto.reportInfo.listInfo.get(i);
						info.checkHightLightByProgress(progressSold);

						// add to list row
						switch (typeRequest) {
						case NVBHGeneralStatisticsView.TYPE_REQUEST_TNPG: {
							TNPGGeneralStatisticsRow row = new TNPGGeneralStatisticsRow(parent, this);
							row.render(info);
							listRow.add(row);
	...
	public void handleModelViewEvent(ModelEvent modelEvent) {
		...
		case ActionEventConstant.GET_GENERAL_STATISTICS_REPORT_INFO_FOR_TNPG:
			// get data
			dto = (GeneralStatisticsTNPGViewDTO) modelEvent.getModelData();
			renderLayout();
			switch (typeRequest) {
			case NVBHGeneralStatisticsView.TYPE_REQUEST_TNPG: {
				requestInsertLogKPI(HashMapKPI.TTTT_BAOCAOCHITIETTIENDONVTT, modelEvent.getActionEvent().startTimeFromBoot);
			}
				break;
			case NVBHGeneralStatisticsView.TYPE_REQUEST_GST_DATE_DETAIL: {
				requestInsertLogKPI(HashMapKPI.GSBH_BAOCAOCHITIETTIENDONGAYTHEONVTT, modelEvent.getActionEvent().startTimeFromBoot);
			}
				break;
			case NVBHGeneralStatisticsView.TYPE_REQUEST_GST_MONTH_DETAIL: {
				requestInsertLogKPI(HashMapKPI.GSBH_BAOCAOCHITIETTIENDOLUYKETHEONVTT, modelEvent.getActionEvent().startTimeFromBoot);
			}
				break;
			default:
				break;
	...
	public boolean onTouch(View v, MotionEvent event) {
		if (v == edDate) {
			if (!edDate.onTouchEvent(event)) {
				parent.fragmentTag = TNPGGeneralStatisticView.TAG;
