...
import com.viettel.dms.view.supervisor.ManagerEquipmentAllRow;
import com.viettel.sabeco.R;
import com.viettel.utils.VTLog;
/**
 * 
 *  man hinh Quan ly thiet bi cua TBHV
 *  @author: Nguyen Huu Hieu
 *  @version: 1.1
 *  @since: 1.0
 */
public class TBHVManagerEquipmentView extends BaseFragment implements
	...
	// main activity
	public static final String TAG = TBHVManagerEquipmentView.class.getName();
	private TBHVActivity parent;	
	private VinamilkTableView tbEquipment;
	private TBHVManagerEquipmentDTO dto;
	...
	public static String[] EQUIPMENT_TABLE_TITLES = { "Mã NPP", "Tên NPP", "Số khách hàng", "Không đạt"};

	public static TBHVManagerEquipmentView getInstance(Bundle b) {
		TBHVManagerEquipmentView instance = new TBHVManagerEquipmentView();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		ViewGroup view = (ViewGroup) inflater.inflate(
				R.layout.layout_manager_equipment, container, false);
		View v = super.onCreateView(inflater, view, savedInstanceState);
		setTitleHeaderView(getString(R.string.TITLE_TBHV_MANAGER_EQUIPMENT));
		initView(v);
		if (isFirst) {			
			getDanhSachThietBi();
		}
	...
	private void getDanhSachThietBi() {
		...
		Bundle b = new Bundle();
		b.putString(IntentConstants.INTENT_PARENT_SHOP_ID, String.valueOf(GlobalInfo.getInstance().getProfile().getUserData().shopId));
//		b.putString(IntentConstants.INTENT_PARENT_SHOP_ID, "125");	
		e.viewData = b;
		e.action = ActionEventConstant.GET_LIST_EQUIPMENT_TBHV;
		e.sender = this;
	...
	private void renderLayout() {
		...
		for (int i = 0, s = dto.arrList.size(); i < s; i++) {
			...
			row = new TBHVManagerEquipmentRow(parent, ACTION_ROW_CLICK);
...
//			}
		...
		ManagerEquipmentAllRow rowTongCong = new ManagerEquipmentAllRow(parent);
...
//			renderLayout();
	...
	private void goToTrackingCabinetStaffView(TBHVManagerEquipmentItem data) {
		...
		e.sender = this;
		Bundle b = new Bundle();
		b.putString(IntentConstants.INTENT_SHOP_ID, data.id);
		b.putString(IntentConstants.INTENT_SHOP_CODE, data.maNVBH);
		b.putString(IntentConstants.INTENT_SHOP_NAME, data.nvbh);
		...
		e.viewData = b;
		e.action = ActionEventConstant.GO_TO_TBHV_MANAGER_EQUIPMENT_DETAIL;
