 ...
 * @since: 1.0
 */
public class TBHVReportProgressSalesFocusRow extends TableRow implements
		OnClickListener {
	 ...
	 * @param context
	 */
	public TBHVReportProgressSalesFocusRow(Context context, View aRow,
			boolean isRowSum, boolean isDetailScreen) {
		super(context);
		tableParent = aRow;
		...
		view = vi.inflate(R.layout.layout_tbhv_progress_report_sales_focus_row,
				this);
	 ...
	 * @throws:
	 */
	public void renderLayoutForTBHVReportFocus(int parentProgress,
			ReportSalesFocusEmployeeInfo item) {
		currentDTO = item;
		if (!this.isRowSum) {
		...
		if (item.listFocusProductItem.size() >0) {
			for (ReportFocusProductItem reportFocusProductItem: item.listFocusProductItem) {
				View child = vi
						.inflate(
								R.layout.layout_tbhv_progress_report_sales_focus_row_item,
								null);
				...
				TextView tvSoldPercent = (TextView) child
						.findViewById(R.id.tvSoldPercent);
				...
				tvPlanMoney.setText(StringUtil
						.parseAmountMoney(reportFocusProductItem.amountPlan));
				tvSoldMoney.setText(StringUtil
						.parseAmountMoney(reportFocusProductItem.amount));
				tvLeftMoney.setText(StringUtil
						.parseAmountMoney(reportFocusProductItem.remain));
				tvSoldPercent.setText(reportFocusProductItem.progress
				...
				if (reportFocusProductItem.amountPlan > 0) {
					if (reportFocusProductItem.amount > 0
							&& reportFocusProductItem.progress < parentProgress) {
						tvSoldPercent.setTextColor(getContext().getResources()
								.getColor(R.color.RED));
					}
					if (reportFocusProductItem.amount > 0
							&& reportFocusProductItem.progress >= parentProgress) {
						tvSoldPercent.setTextColor(ImageUtil
				...
				if (reportFocusProductItem.amountPlan <= 0) {
					...
					if (reportFocusProductItem.amount == 0 && parentProgress == 0) {
			...
			TextView tvSoldPercent = (TextView) child
					.findViewById(R.id.tvSoldPercent);
			tvPlanMoney.setText(StringUtil.parseAmountMoney("0"));
			tvSoldMoney.setText(StringUtil.parseAmountMoney("0"));
			tvLeftMoney.setText(StringUtil.parseAmountMoney("0"));
			tvSoldPercent.setText("0"
					+ StringUtil.getString(R.string.TEXT_SYMBOL_PERCENT));
			llMHTT.addView(child, param);
	...
	public void onClick(View v) {
		...
		if (v == tvStaffOwnerName && this.listener != null) {
			this.listener.handleVinamilkTableRowEvent(
					ACTION_SHOW_REPORT_PROGRESS_SALE_FOCUS_DETAIL_CLICK_NPP,
					tableParent, currentDTO);
		} else if (v == tvStaffName && this.listener != null) {
			this.listener.handleVinamilkTableRowEvent(
					ACTION_SHOW_REPORT_PROGRESS_SALE_FOCUS_DETAIL_CLICK_GSNPP,
					tableParent, currentDTO);
		}
	}
