...
OnEventControlListener, OnClickListener, VinamilkTableListener{
	public static final String TAG = IntroduceProductView.class.getName();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		setTitleHeaderView(StringUtil.getString(R.string.TITLE_VIEW_INTRODUCE_PRODUCT));
	...
	*/
	...
	private void openMeida(String path, int mediaType) {
		...
		if(mediaType == MediaItemDTO.MEDIA_IMAGE){
			...
			//thêm KPI
			requestInsertLogKPI(HashMapKPI.NVBH_TAIANH, startTimeDownloadImage);
			startTimeDownloadImage = 0;
		}else{
			intent.setDataAndType(Uri.parse(path), "video/mp4");
			//thêm KPI
			requestInsertLogKPI(HashMapKPI.NVBH_TAIVIDEO, startTimeDownloadVideo);
			startTimeDownloadVideo = 0;
		}
	...
	private class DownloadTask extends AsyncTask<String, String, String> {
		...
		protected void onProgressUpdate(String... values) {
			...
			if (values != null) {
				...
				//check điều kiện màn hình vẫn còn tồn tại
				if(IntroduceProductView.this != null && IntroduceProductView.this.isVisible()){
	...
	public void onClick(View v) {
		...
		if(v == imgViewFull){
			...
			if(GlobalUtil.isFileExistsInDirectory(new File(ExternalStorage.DMS_SABECO_FOLDER + "PRODUCTS/"), mediaName)){
				...
				if (curentImage.mediaType == MediaItemDTO.MEDIA_IMAGE) {
					startTimeDownloadImage = SystemClock.elapsedRealtime();
				} else {
					startTimeDownloadVideo = SystemClock.elapsedRealtime();
				}
				//download moi ve
	...
	public void handleModelViewEvent(ModelEvent modelEvent) {
		...
		renderLayout(inDTO);
		requestInsertLogKPI(HashMapKPI.NVBH_GIOITHIEUSANPHAM, modelEvent.getActionEvent().startTimeFromBoot);
	}
	...
	*  @throws:
	...
	long startTimeDownloadImage = 0;
	long startTimeDownloadVideo = 0;
