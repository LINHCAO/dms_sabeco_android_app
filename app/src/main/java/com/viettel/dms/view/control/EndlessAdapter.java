...
abstract public class EndlessAdapter extends AdapterWrapper {
  ...
  public boolean continueLoad = true;
  /**
 ...
 public EndlessAdapter(BaseAdapter wrapped) {
  ...
  public int getViewTypeCount() {
    ...
    */
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (position + 4 >= super.getCount() 
				&& keepOnAppending.get()) {
			VTLog.i("EndlessAdapter", "-----  call AppendTask :  " + continueLoad);
			if (continueLoad) {
				new AppendTask().execute();
			}
		}

		if (super.getCount() > 0 && position == super.getCount() && keepOnAppending.get()) {
			if (pendingView == null) {
  ...
  protected boolean onException(View pendingView, Exception e) {
    VTLog.e("EndlessAdapter", "Exception in cacheInBackground()", e);
