 ...
 * @author dungdq3
 * 
 * @author : DungDQ3 since : 1.0 version : 1.0
 */
public class CustomerRouteViewPG extends FragmentMapView implements OnItemSelectedListener {
	public static final String TAG = CustomerRouteViewPG.class.getName();
	...
	public static CustomerRouteViewPG getInstance() {
		CustomerRouteViewPG f = new CustomerRouteViewPG();
	...
	private void getCustomerInVisitPlan() {
		...
		b.putInt(IntentConstants.INTENT_ROLE_TYPE, 1);
		e.viewData = b;
	...
	private void showCustomerInfoPopup(int[] data, boolean isClikInCustomer) {
		...
		CustomerPopupView popupInfoView = new CustomerPopupView(parent, "", Constants.TYPE_TT);
		...
		for (int i = 0; i < size; i++) {
			...
			if (cus.aCustomer.getLat() > 0 && cus.aCustomer.getLng() > 0) {
				...
				if (!getVisitPlan(arrLineChoose[spiner.getSelectedItemPosition()]).equals(
					...
					else if ((cus.visitStatus == VISIT_STATUS.VISITED_FINISHED || cus.visitStatus == VISIT_STATUS.VISITED_CLOSED)) {
						showMarkerInMap(cus.aCustomer.getLat(), cus.aCustomer.getLng(), i, cus.isOr, cus.seqInDayPlan,
	...
	private void gotoCustomerInfo(String customerId) {
		...
		bundle.putString(IntentConstants.INTENT_PARENT_PREFIX_TITLE, "02-02");
	...
	private void processOrder(CustomerListItem item) {
		if (item.isOr == 0) {// KH trong tuyen
			...
			if (isValidDistanceToVoteDisplayProgAndCheckRemain(item) && item.isHaveDisplayProgramNotYetVote) {
				gotoVoteDisplayPresentProduct(item);
			} else if (isValidDistanceToVoteDisplayProgAndCheckRemain(item) && item.isHaveSaleOrder && !item.isTodayCheckedRemain) {
			...
			if (item.visitStatus == VISIT_STATUS.VISITED_FINISHED) {
				parent.initCustomerNameOnMenuVisit(item.aCustomer.customerCode, item.aCustomer.customerName);
			} else {
				parent.initMenuVisitCustomer(item.aCustomer.shortCode, item.aCustomer.customerName);
	...
	public void handleModelViewEvent(ModelEvent modelEvent) {
		...
		case ActionEventConstant.GET_CUSTOMER_LIST_FOR_ROUTE:
			...
			drawCustomerLocation();
			requestInsertLogKPI(HashMapKPI.TTTT_LOTRINH, modelEvent.getActionEvent().startTimeFromBoot);
			break;
