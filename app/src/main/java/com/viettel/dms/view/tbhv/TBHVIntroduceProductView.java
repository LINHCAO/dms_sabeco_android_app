...
/**
 * 
 * MH gioi thieu sp TBHV
 * 
 * @author: Nguyen Thanh Dung
 * @version: 1.1
 * @since: 1.0
 */
public class TBHVIntroduceProductView extends BaseFragment implements
		OnEventControlListener, OnClickListener, VinamilkTableListener {
	public static final String TAG = TBHVIntroduceProductView.class.getName();
	// frame image view full
	private FrameLayout flImageViewFull;
	// man hinh video
	private ImageView imgViewFull;
	private TextView tvProductName;// Ten sp
	private int selectIndex;// Index hinh anh/video duoc chon
	private LinearLayout llGallery;// layout ds hinh anh thumb
	private WebView wvIntroduce;// Noi dung gioi thieu
	// category for list location search
	// category adapter
	MediaGalleryAdapter mediaAdapter;
	HorizontalListView gallery;// list view hinh anh
	...
	public static TBHVIntroduceProductView getInstance(Bundle data) {
		// if (instance == null) {
		TBHVIntroduceProductView instance = new TBHVIntroduceProductView();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		setTitleHeaderView(StringUtil
				.getString(R.string.TITLE_VIEW_TBHV_INTRODUCE_PRODUCT));
	...
	private void requestIntroduceProduct(String productId) {
		...
		e.viewData = data;
		e.sender = this;
		e.action = ActionEventConstant.GET_TBHV_INTRODUCE_PRODUCT;
	...
	private class DownloadTask extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... params) {
			...
			String path = Constants.STR_BLANK;
			try {
		...
		protected void onProgressUpdate(String... values) {
			...
			if (values != null) {
				...
				if(TBHVIntroduceProductView.this != null && TBHVIntroduceProductView.this.isVisible()){
	...
	public void onClick(View v) {
		...
		if (v == imgViewFull) {
			...
			if (GlobalUtil.isFileExistsInDirectory(new File(
					ExternalStorage.DMS_SABECO_FOLDER + "PRODUCTS/"),
					mediaName)) {
