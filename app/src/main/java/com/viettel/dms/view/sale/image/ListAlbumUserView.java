...
/**
 * Man hinh chua cac album cua mot khach hang (Role NVBH)
 * 
 * @author: QuangVT
 * @version: 1.0 
 */
public class ListAlbumUserView extends BaseFragment implements OnItemClickListener, OnClickListener,
		OnItemSelectedListener , OnImageTakingPopupListener { 
 
	public static final String TAG = ListAlbumUserView.class.getName(); // Tag
	...
	private static final int TAB_INDEX_GSBH = 1;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		ViewGroup view = (ViewGroup) inflater.inflate(R.layout.layout_album_user_view, container, false);
	...
	private void initMenuActionBar() {
		if(parent instanceof SupervisorActivity || parent instanceof SalesPersonActivity) {
...
//		} else {
//			addMenuItem(StringUtil.getString(R.string.TEXT_LABLE_REVIEWS), R.drawable.icon_note, MENU_REVIEW);
//		}
			addMenuItem(StringUtil.getString(R.string.TEXT_FEEDBACK), R.drawable.icon_reminders, MENU_LIST_FEEDBACKS);
	...
	public void handleModelViewEvent(ModelEvent modelEvent) {
		...
		case ActionEventConstant.GO_TO_LIST_ALBUM_USER: {
			...
			if (GlobalInfo.getInstance().getProfile().getUserData().chanelObjectType == UserDTO.TYPE_VALSALES
				...
				requestInsertLogKPI(HashMapKPI.NVBH_DANHSACHALBUM, modelEvent.getActionEvent().startTimeFromBoot);
			} else if (GlobalInfo.getInstance().getProfile().getUserData().chanelObjectType == UserDTO.TYPE_GSNPP) {
				requestInsertLogKPI(HashMapKPI.GSNPP_DANHSACHALBUM, modelEvent.getActionEvent().startTimeFromBoot);
			}
		...
		case ActionEventConstant.UPLOAD_PHOTO_TO_SERVER: {
			...
			if (GlobalInfo.getInstance().getProfile().getUserData().chanelObjectType == UserDTO.TYPE_VALSALES
					|| GlobalInfo.getInstance().getProfile().getUserData().chanelObjectType == UserDTO.TYPE_PRESALES) {
				requestInsertLogKPI(HashMapKPI.NVBH_CHUPHINHTAIDIEMBAN, modelEvent.getActionEvent().startTimeFromBoot);
			} else if (GlobalInfo.getInstance().getProfile().getUserData().chanelObjectType == UserDTO.TYPE_GSNPP) {
			...
			// tu chi tiet album
			PhotoThumbnailListView orderFragment = (PhotoThumbnailListView) getActivity().getFragmentManager()
					.findFragmentByTag(PhotoThumbnailListView.TAG);
	...
	public class ImageAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView, ViewGroup pt) {
			...
			if (!StringUtil.isNullOrEmpty(list.get(position).getThumbUrl())) {
				...
				holder.imageView.setTag(Constants.STR_BLANK);
			}
...
//		}
		...
		try {
			...
			if(!StringUtil.isNullOrEmpty(model.getCustomer().customerCode)) {
