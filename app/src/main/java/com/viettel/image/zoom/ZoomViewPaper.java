 ...
 * data. You supply an implementation of a {@link ListFragmentAdapter} to
 ...
 * </p>
 */
public class ZoomViewPaper extends ViewGroup {
	private static final String TAG = "PhucNT4";
	...
	public ZoomViewPaper(Context context) {
		super(context);

	}

	public ZoomViewPaper(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	void populate() {
		...
		if (mPopulatePending) {
			if (DEBUG)
				VTLog.i(TAG, "populate is pending, skipping for now...");
			return;
		}
		...
		if (DEBUG)
			VTLog.v(TAG, "populating: startPos=" + startPos + " endPos=" + endPos);
		...
		for (int i = 0; i < mItems.size(); i++) {
			...
			if ((ii.position < startPos || ii.position > endPos) && !ii.scrolling) {
				if (DEBUG)
					VTLog.i(TAG, "removing: " + ii.position + " @ " + i);
				mItems.remove(i);
				i--;
				mAdapter.destroyItem(this, ii.position, ii.object);
				...
				while (lastPos <= endPos && lastPos < ii.position) {
					if (DEBUG)
						VTLog.i(TAG, "inserting: " + lastPos + " @ " + i);
		...
		if (DEBUG) {
			VTLog.i(TAG, "Current page list:");
			for (int i = 0; i < mItems.size(); i++) {
				VTLog.i(TAG, "#" + i + ": page " + mItems.get(i).position);
			}
		}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		mChildWidthMeasureSpec = MeasureSpec.makeMeasureSpec(getMeasuredWidth() - getPaddingLeft() - getPaddingRight(),
				MeasureSpec.EXACTLY);
		mChildHeightMeasureSpec = MeasureSpec.makeMeasureSpec(getMeasuredHeight() - getPaddingTop()
		...
		for (int i = 0; i < size; ++i) {
			...
			if (child.getVisibility() != GONE) {
				if (DEBUG)
					VTLog.v(TAG, "Measuring #" + i + " " + child + ": " + mChildWidthMeasureSpec);
				child.measure(mChildWidthMeasureSpec, mChildHeightMeasureSpec);
			}
		}
	}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (child.getVisibility() != GONE && (ii = infoForChild(child)) != null) {
				...
				if (DEBUG)
					VTLog.v(TAG, "Positioning #" + i + " " + child + " f=" + ii.object + ":" + childLeft + "," + childTop
	...
	public void computeScroll() {
		if (DEBUG)
			VTLog.i(TAG, "computeScroll: finished=" + mScroller.isFinished());
		if (!mScroller.isFinished()) {
			if (mScroller.computeScrollOffset()) {
				if (DEBUG)
					VTLog.i(TAG, "computeScroll: still scrolling");
				int oldX = getScrollX();
				int oldY = getScrollY();
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (action != MotionEvent.ACTION_DOWN) {
			if (mIsBeingDragged) {
				if (DEBUG)
					VTLog.v(TAG, "Intercept returning true!");
				return true;
			}
			if (mIsUnableToDrag) {
				if (DEBUG)
					VTLog.v(TAG, "Intercept returning false!");
				return false;
			}
		}
		...
		case MotionEvent.ACTION_MOVE: {
			...
			if (xDiff > mTouchSlop && xDiff > yDiff) {
				...
				if (yDiff > mTouchSlop) {
					...
					if (DEBUG)
						VTLog.v(TAG, "Starting unable to drag!");
		...
		case MotionEvent.ACTION_DOWN: {
			...
			if (DEBUG)
				VTLog.v(TAG, "Down at " + mLastMotionX + "," + mLastMotionY + " mIsBeingDragged=" + mIsBeingDragged
